...
# 1. generate race
n <- 1000
df <- data.frame(race= c(rep(0, 750), rep(1, 250)))
...
print(nrow(coeffs_df)) # a total of 34,202 cases

sub_df <- coeffs_df[coeffs_df["coef_u.Pr...z.."] < .05 & coeffs_df["coef_a.Pr...z.."] < .05, ]
nrow(sub_df) # there are a total of 10,488 cases in which the unadjusted and adjusted coefficients of race are statistically significant at 5% level of significance
