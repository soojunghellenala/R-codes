  ...
  for (b in b_values) {
    for (c in c_values) {
      for (d in d_values) {
        ...
        if (!is.na(fairness[1]) && !is.na(fairness[2]) && fairness[1] > 22 && fairness[2] > 30) {
...
for(i in 1:nrow(combinations)){
  ...
  glm_Y0 <- summary(glm(Y_h ~ race, sub = Y == 0, family = binomial(link = 'logit'), data = df)) # Adjusting for Y by fixing the value of Y at 0
  glm_Y1 <- summary(glm(Y_h ~ race, sub = Y == 1, family = binomial(link = 'logit'), data = df)) # Adjusting for Y by fixing the value of Y at 1
  ...
  coef_y0 <- as.data.frame(t(glm_Y0$coefficients['race', ]))
  coef_y1 <- as.data.frame(t(glm_Y1$coefficients['race', ]))
...
print(nrow(coeffs_df)) # a total of 3015 cases

sub_df <- coeffs_df[coeffs_df["coef_u.Pr...z.."] < .05 & coeffs_df["coef_a.Pr...z.."] < .05, ]
nrow(sub_df) # there are a total of 116 cases in which the unadjusted and adjusted coefficients of race are statistically significant at 5% level of significance
